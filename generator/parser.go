...
func parse_cfg(sheet *xlsx.Sheet, row_cnt int, cfg *Cfg_t, errvec *tool.Errvec_t) bool {
	// 文件名称
	cfg.En_name = tool.Getcellstr(sheet, cfg.row_file_cfg_begin+int(cfg_row_1), int(cfg_col_1))

	// 最大数据条数
	cfg.Maxcnt = tool.Getcellint(sheet, cfg.row_file_cfg_begin+int(cfg_row_1), int(cfg_col_2))
...
func parse_field(sheet *xlsx.Sheet, cfg *Cfg_t, col int, field *Field_t, errvec *tool.Errvec_t) bool {
	// 取出字段英文名
	field.En_name = strings.Trim(tool.Getcellstr(sheet, cfg.row_fields_begin+int(field_row_en_name), col), " \t")
	if field.En_name == "" {
		// 碰到空字段则停止解析
		return true
	}
	...
	// 取出字段类型
	var fieldtype_cell string = strings.Trim(tool.Getcellstr(sheet, cfg.row_fields_begin+int(field_row_data_type), col), " \t")
	field.Fieldtype = type_name_2_enum(&fieldtype_cell)

	// 取出字段属性
	var fieldattr_cell string = strings.Trim(tool.Getcellstr(sheet, cfg.row_fields_begin+int(field_row_is_unique), col), " \t")
	...
	field.Cn_name = strings.Trim(tool.Getcellstr(sheet, cfg.row_data_begin, col), " \t")
...
func parse_data(sheet *xlsx.Sheet, row_cnt int, col_cnt int, cfg *Cfg_t, errvec *tool.Errvec_t) bool {
	...
	for row := int(cfg.row_data_begin + 1); row <= row_cnt; row++ {
		...
		for col := 1; col <= n_field; col++ {
			r[col-1] = tool.Getcellstr(sheet, row, col)
...
func parse_sheet(sheet *xlsx.Sheet, cfg *Cfg_t, errvec *tool.Errvec_t, parse_option Enum_parse_option) bool {
	// 1. 获取excel表的行数和列数
	var row_cnt int = tool.Getrowcnt(sheet)
	var col_cnt int = tool.Getcolcnt(sheet)
